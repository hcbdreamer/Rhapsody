...
//## operation getProduct(int)
#include "Product.h"
//## dependency BuyOneGetOneFree
#include "BuyOneGetOneFree.h"
//#[ ignore
#define CashRegisterPkg_ProductDatabase_ProductDatabase_SERIALIZE OM_NO_OP

#define CashRegisterPkg_ProductDatabase_getProduct_SERIALIZE aomsmethod->addAttribute("aCode", x2String(aCode));
//#]
...
ProductDatabase::ProductDatabase() : itsProduct() {
    ...
    ISpecialOffer* todaysOffer = new BuyOneGetOneFree;
    addItsProduct(12344,new Product(12344,"Coconuts",180));
    addItsProduct(12345,new Product(12345,"Lychees",250));
    addItsProduct(12346,new Product(12346,"Kiwis",120,todaysOffer));
...
Product* ProductDatabase::getProduct(int aCode) {
    NOTIFY_OPERATION(getProduct, getProduct(int), 1, CashRegisterPkg_ProductDatabase_getProduct_SERIALIZE);
    //#[ operation getProduct(int)
    return getItsProduct(aCode);
    //#]
}
